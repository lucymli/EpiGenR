...
```{r time_series_from_tree2, eval=FALSE, echo=FALSE}
gen_data <- time_series_from_tree(subtree, step_size=data.dt)
```

```{r gdata}
gen_data <- EpiGenR::coal.intervals.in.discrete.time(EpiGenR::coalescent.intervals.datedPhylo(subtree), data.dt)
```

<!--To ensure that the epidemiological and genetic data start and finish at the same time, 
run:-->

```{r alldata, eval=FALSE, echo=FALSE}
#EpiGenR::align_epi_gen_data(epi_data, gen_data, data.dt, sampled.sim.outbreak$sampled_individuals)
```
...
param_list <- create_params_list(
  ...
  transform=c(NA, NA, "inverse", NA, NA), # The algorithm will estimate the value of the transformed parameter
  prior=c("unif", "unif", "unif", "beta", "unif"), # Prior distribution
  prior_params=list(c(1.0, 100.0), c(1.0, 10000.0), c(1.0, 30.0), c(1.0, 3.0)), # Parameters for the prior distribution
  proposal_params=list(c(0.5, 1.0, 100.0), c(0.1, 0.001, 10), c(1.0, 1.0, 30.0), c(0.05, 0.0, 1.0)),  
...
mcmc_options <- create_mcmc_options (
  particles=1000, iterations=1000, log_every=1, pfilter_every=20, 
...
input_files <- EpiGenR::generate_cpp_input_files(
  dt=dt, params=param_list, initial_states=list(S=S, I=1, R=0),
  data=list(epi_data, gen_data),
